...
Quite a lot has been made in the media of the gender differences among users of the New York Citibike scheme. There are two types of users: long-term `Subscribers` to the service, and short term `Customers` who might only buy a weekly pass. Limited information is collected from pass buyers, but `Subscribers` are asked for their gender and `birth_year`, so it is possible to drill into this question a little.
...
Ok, so women have definitely using the service less since it started, but how has that changed over time?

Looking at the plot below, you can see the seaonal did in users, but there's nothing to indicate that the pro

```{r gender_month_plot,cache=TRUE,echo=TRUE,message=FALSE,warning=FALSE,include=TRUE}
